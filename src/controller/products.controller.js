...
const saveProduct = async (req, res) => {        // Guarda un prodcuto nuevo
    ...
    if ( !name || !price || !urlImage || !description || !code || !stock ) {                          // Comprobamos que el cuerpo este completo
        ...
        try {
            ...
            product.timeStamp = `${d.getDate()}/${d.getMonth() + 1}/${d.getFullYear()} ${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}` // Insertamos la fecha
        ...
        try {
            ...
            for ( let i = 0; i < dbData.length; i++) {                                          // Recorremos el array, en caso que coincidan los ID lo actualizamos
                if (dbData[i].id == id) {
                    ...
                    dbData[i].timeStamp = `${d.getDate()}/${d.getMonth() + 1}/${d.getFullYear()} ${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}`
                    contador += 1
                    break
                }
            }
